...
class ParserResultJoin : public AParserResult {
  public:
    ...
    int checkJoinParams(std::vector <std::string> messageVector);
    ...
    bool getLeaveAllChansFlag(void) const;
  ...
  private:
    ...
    bool _leaveAllChans; // if the flag is TRUE -> the server has to remove the client from ALL the channels!
};
