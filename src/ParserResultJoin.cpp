...
bool ParserResultJoin::getLeaveAllChansFlag(void) const{
...
int ParserResultJoin::checkJoinParams(std::vector <std::string> messageVector) {
    ...
    this->_leaveAllChans = false;
    joinChannelsVec = stringToVec(messageVector[1], ',');
    for(size_t i = 0; i < _joinParamsVec.size(); i++) {
        if(!isValidChanName(_joinParamsVec.at(i))) {
